...
using RBBot.Core.Exchanges.Kraken.Common;
using RBBot.Core.Exchanges.Kraken.Models;
...
namespace RBBot.Core.Exchanges.Kraken.Common
{
    ...
    public partial class KrakenClient : IDisposable
    {
        ...
        private static readonly Dictionary<RateLimit, TierInfo> TierInfo
            = new Dictionary<RateLimit, TierInfo>(3)
            {
                [RateLimit.Tier2] = new TierInfo() { Limit = 15, DecreaseTime = TimeSpan.FromSeconds(3) },
                [RateLimit.Tier3] = new TierInfo() { Limit = 20, DecreaseTime = TimeSpan.FromSeconds(2) },
                [RateLimit.Tier4] = new TierInfo() { Limit = 20, DecreaseTime = TimeSpan.FromSeconds(1) }
            };
